...
struct JobCheckResult
{
     ...
     */
    JobCheckResult(const std::string& file, JobStatus stat) : filename(file), status(stat) {}
};
...
struct CheckSummary
{
     ...
     */
    CheckSummary()
        : total_files(0), processed_files(0), matched_files(0), moved_files(0), failed_moves(0), execution_time(0.0)
    {}
};
...
private:
    ...
    std::string
    read_file_unified(const std::string& filename, FileReadMode mode = FileReadMode::TAIL, size_t tail_lines = 10);
