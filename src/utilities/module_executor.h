 ...
 * @brief Command execution functions for ComChemKit
 * @author Le Nhan Pham
 * @date 2025
 *
 * This header defines the module execution functions that implement
 * the actual functionality for each supported command in ComChemKit.
...
/**
 * @brief Execute the thermo command for advanced thermodynamic analysis
 * @param context Configured command context with thermo parameters
 * @return Exit code: 0 for success, non-zero for various error conditions
 *
 * Performs comprehensive thermodynamic analysis supporting multiple quantum chemistry programs:
 * - Gaussian, ORCA, GAMESS, NWChem, CP2K, VASP, xTB output processing
 * - Advanced vibrational treatments (Harmonic, Truhlar, Grimme, Minenkov)
 * - Temperature and pressure scanning capabilities
 * - Symmetry detection and rotational analysis
