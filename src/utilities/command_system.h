 ...
 * @brief Command parsing and execution system for ComChemKit
 * @author Le Nhan Pham
 * @date 2025
 *
 * This header defines the command system architecture for the ComChemKit
...
/**
 * @enum CommandType
 * @brief Enumeration of available command types in ComChemKit
 *
 * This enumeration defines all supported commands that can be executed by the
 * ComChemKit application. Each command type corresponds to a specific
 ...
 * needed to execute any command in the ComChemKit. It serves as
 ...
 * for the ComChemKit application. It handles:
...
private:
    ...
    /**
     * @brief Parse options specific to thermo command
     * @param context CommandContext to populate
     * @param i Current argument index (modified by reference)
     * @param argc Total number of arguments
     * @param argv Argument array
     *
     * Handles thermo-specific options like --temp-scan, --pressure-scan, --low-vib-treatment.
     */
    static void parse_thermo_options(CommandContext& context, int& i, int argc, char* argv[]);
