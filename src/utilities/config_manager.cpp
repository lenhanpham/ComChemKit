...
void ConfigManager::initialize_default_values()
{
    ...
    config_values["input_extensions"] =
        ConfigValue(".com,.gjf,.gau", "Comma-separated list of input file extensions", "general");
    config_values["output_extensions"] =
        ConfigValue(".log,.out", "Comma-separated list of output file extensions", "general");
    ...
    config_values["done_directory_suffix"] =
        ConfigValue("done", "Default suffix for completed jobs directory", "job_checker");
    config_values["error_directory_name"] =
        ConfigValue("errorJobs", "Default directory name for error jobs", "job_checker");
    config_values["pcm_directory_name"] =
        ConfigValue("PCMMkU", "Default directory name for PCM failed jobs", "job_checker");
    config_values["show_error_details"] = ConfigValue("false", "Show error details by default", "job_checker");
    config_values["move_related_files"] =
        ConfigValue("true", "Move related .gau/.chk files with .log files", "job_checker");
    config_values["create_subdirectories"] =
        ConfigValue("true", "Create subdirectories for job organization", "job_checker");
    ...
    // Output settings
    config_values["results_filename_template"] =
        ConfigValue("{dirname}.results", "Template for results filename", "output");
...
bool ConfigManager::find_config_file()
{
    ...
    if (!exe_dir.empty())
    {
        search_paths.push_back(exe_dir + "/" + DEFAULT_CONFIG_FILENAME);
        search_paths.push_back(exe_dir + "/" + ALT_CONFIG_FILENAME);
    }
...
#ifndef _WIN32
    search_paths.push_back("/etc/comchemkit/" + ALT_CONFIG_FILENAME);
...
{
    std::cout << "# ComChemKit Configuration File" << std::endl;
    std::cout << "# Save this as .comchemkit.conf in your home directory or current working directory"
              << std::endl;
    std::cout << "#" << std::endl;
...
namespace ConfigUtils
{
    ...
    std::string get_config_search_paths()
    {
        ...
        if (!exe_dir.empty())
        {
            oss << "  3. " << exe_dir << "/" << DEFAULT_CONFIG_FILENAME << "\n";
            oss << "  4. " << exe_dir << "/" << ALT_CONFIG_FILENAME << "\n";
        }
        ...
        if (!home.empty())
        {
            int next_num = exe_dir.empty() ? 3 : 5;
            oss << "  " << next_num << ". " << home << "/" << DEFAULT_CONFIG_FILENAME << "\n";
            oss << "  " << (next_num + 1) << ". " << home << "/" << ALT_CONFIG_FILENAME << "\n";
        }

#ifndef _WIN32
        int sys_num = exe_dir.empty() ? (home.empty() ? 3 : 5) : (home.empty() ? 5 : 7);
        oss << "  " << sys_num << ". /etc/comchemkit/" << ALT_CONFIG_FILENAME << "\n";
        oss << "  " << (sys_num + 1) << ". /usr/local/etc/" << ALT_CONFIG_FILENAME << "\n";
#endif
